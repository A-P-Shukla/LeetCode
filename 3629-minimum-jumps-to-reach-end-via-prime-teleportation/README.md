...
1.  **Adjacent Edges:** $i \leftrightarrow i+1$ and $i \leftrightarrow i-1$.
2.  **Teleportation Edges:** If $nums[i]$ is prime $p$, there are directed edges from $i$ to every $j$ where $nums[j]$ is a multiple of $p$.
...
-   **Time Complexity:** $O(M \log \log M + N \log M)$, where $M$ is the maximum value in `nums` ($10^6$) and $N$ is the length of the array ($10^5$).
